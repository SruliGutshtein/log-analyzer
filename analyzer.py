from checks import is_external_ip, is_sensitive_port, is_large_size, is_night_activity
...
def identifying_large_sizes(packets):
    ...
    return [packet for packet in packets if is_large_size(packet[-1])]

def add_size_tag(packets):
    """פונקציה שעוברת על רשימה ועושה רשימה חדששה עם תיוג אם הגודל חשוד"""
    return [packet + ["LARGE"] if is_large_size(packet[-1]) else packet + ["NORMAL"] for packet in packets]
